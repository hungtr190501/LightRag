...
def _semantic_split_legal_text(
    content: str,
) -> list[tuple[str, dict[str, Any]]]:
    """
    Split legal text by semantic markers (Điều, Khoản, Điểm, etc.)
    Returns list of (text_segment, metadata) where metadata includes
    semantic_type, semantic_number, and hierarchy info.
    """
    import re

    # Patterns for Vietnamese legal structure
    # Order matters: from larger to smaller units
    semantic_patterns = [
        # Điều (Article): "Điều 1.", "Điều 15." etc.
        (r"(?:^|\n)(Điều\s+\d+[.\s])", "dieu"),
        # Khoản (Clause): "1.", "2.", "3." (at start of line)
        (r"(?:^|\n)(\d+\.\s)", "khoan"),
        # Điểm (Point): "a)", "b)", "c)" etc.
        (r"(?:^|\n)([a-z]\)\s)", "diem"),
    ]
    ...
    while i < len(lines):
        ...
        for pattern, sem_type in semantic_patterns:
            if re.match(pattern, line):
                ...
                if current_pos < i:
                    seg_text = "\n".join(lines[current_pos:i])
                    if seg_text.strip():
                        segments.append((seg_text, current_metadata.copy()))

                # Extract semantic number
                match = re.search(r"\d+|[a-z]", line)
                sem_number = match.group(0) if match else ""

                current_metadata = {
                    "semantic_type": sem_type,
                    "semantic_number": sem_number,
    ...
    if current_pos < len(lines):
        ...
        if seg_text.strip():
            segments.append((seg_text, current_metadata.copy()))
            # Update end_line for last segment
            if segments:
                segments[-1][1]["end_line"] = len(lines)
...
def chunking_by_token_size(
    ...
    semantic_chunking: bool = False,  # NEW: enable semantic chunking for legal text
    use_langchain_semantic_chunker: bool = False,
) -> list[dict[str, Any]]:
    def _normalize_newlines(text: str) -> str:
        return text.replace("\r\n", "\n").replace("\r", "\n")

    def _attach_line_spans_to_chunks(
        original_text: str, chunk_results: list[dict[str, Any]]
    ) -> None:
        """Attach best-effort line_start/line_end (1-based) to chunks in place."""
        normalized_source = _normalize_newlines(original_text)
        ...
        for chunk in chunk_results:
            ...
            primary_start = max(0, last_search_cursor - len(normalized_chunk))
            start_pos = normalized_source.find(normalized_chunk, primary_start)

            # Fallback for repeated/overlapped segments or slight decoding drift.
            if start_pos < 0:
                start_pos = normalized_source.find(normalized_chunk)

            if start_pos < 0:
                continue

            end_pos = start_pos + len(normalized_chunk)
            chunk["line_start"] = normalized_source.count("\n", 0, start_pos) + 1
            chunk["line_end"] = (
                normalized_source.count("\n", 0, max(end_pos - 1, start_pos)) + 1
            )
            last_search_cursor = end_pos

    # Optional LangChain semantic chunking path.
    if semantic_chunking and use_langchain_semantic_chunker:
        try:
            ...
            breakpoint_type = os.getenv(
                "LANGCHAIN_SEMANTIC_BREAKPOINT_TYPE", "percentile"
            )
            breakpoint_amount = float(
                os.getenv("LANGCHAIN_SEMANTIC_BREAKPOINT_AMOUNT", "95")
            )
            ...
            lc_chunks = [doc.page_content.strip() for doc in lc_docs if doc.page_content.strip()]

            if not lc_chunks:
                logger.warning(
                    "LangChain SemanticChunker returned no chunks, falling back to built-in semantic chunker"
                )
            else:
                logger.info(
                    "Using LangChain SemanticChunker for chunking: produced %d semantic chunks",
                    len(lc_chunks),
                )
                results = []
                chunk_index = 0
                for chunk_text in lc_chunks:
                    chunk_tokens = tokenizer.encode(chunk_text)
                    if len(chunk_tokens) > chunk_token_size:
                        for start in range(
                            0, len(chunk_tokens), chunk_token_size - chunk_overlap_token_size
                        ):
                            chunk_content = tokenizer.decode(
                                chunk_tokens[start : start + chunk_token_size]
                            )
                            results.append(
                                {
                                    "tokens": min(
                                        chunk_token_size, len(chunk_tokens) - start
                                    ),
                ...
                _attach_line_spans_to_chunks(content, results)
                return results
        except ImportError:
            logger.warning(
                "LangChain semantic chunker requested but dependencies are missing. "
                "Install 'langchain-experimental' and 'langchain-openai'. "
                "Falling back to built-in semantic chunker."
            )
        except Exception as e:
            logger.warning(
                "LangChain semantic chunking failed (%s). Falling back to built-in semantic chunker.",
                str(e),
            )

    # Semantic chunking for legal text (Điều, Khoản, Điểm)
    if semantic_chunking:
        sem_segments = _semantic_split_legal_text(content)
        results = []
        chunk_index = 0
        for seg_text, metadata in sem_segments:
            seg_tokens = tokenizer.encode(seg_text)
            if len(seg_tokens) > chunk_token_size:
                # Split large semantic segment by token size with overlap
                for start in range(
                    0, len(seg_tokens), chunk_token_size - chunk_overlap_token_size
                ):
                    chunk_content = tokenizer.decode(
                        seg_tokens[start : start + chunk_token_size]
                    )
                    chunk_data = {
                        "tokens": min(chunk_token_size, len(seg_tokens) - start),
                        "content": chunk_content.strip(),
                        "chunk_order_index": chunk_index,
                        "semantic_type": metadata.get("semantic_type"),
                        "semantic_number": metadata.get("semantic_number"),
                    }
                    # Compute line spans for this sub-chunk (approximate)
                    if metadata.get("start_line"):
                        # Estimate line position based on segment start + proportion
                        chunk_data["line_start"] = metadata["start_line"]
    ...
    # Original chunking logic (non-semantic)
...
async def _build_context_str(
    ...
    def _format_reference(ref: dict) -> str:
        """Format a reference entry with semantic info if available."""
        ref_id = ref["reference_id"]
        file_path = ref["file_path"]
        sem_info = ""

        # Semantic info has priority
        sem_type = ref.get("semantic_type")
        sem_num = ref.get("semantic_number")
        if sem_type and sem_num:
            if sem_type == "dieu":
                sem_info = f" (Điều {sem_num})"
            elif sem_type == "khoan":
                sem_info = f" (Khoản {sem_num})"
            elif sem_type == "diem":
                sem_info = f" (Điểm {sem_num})"
        elif ref.get("chunk_order_index") is not None:
            sem_info = f" (đoạn {int(ref['chunk_order_index']) + 1})"

        return f"[{ref_id}] {file_path}{sem_info}"
