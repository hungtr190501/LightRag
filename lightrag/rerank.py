...
async def generic_rerank_api(
    ...
    async def _post_with_payload(
        request_payload: Dict[str, Any], suppress_error_log: bool = False
    ) -> Dict[str, Any]:
        """Execute rerank request and return response JSON or raise detailed error."""
        async with aiohttp.ClientSession() as session:
            async with session.post(base_url, headers=headers, json=request_payload) as response:
    ...
    async def _request_with_object_documents_fallback(
        base_payload: Dict[str, Any], docs: List[str]
    ) -> Dict[str, Any]:
        """Try the default payload; if API expects document objects, retry automatically."""
        try:
            return await _post_with_payload(base_payload, suppress_error_log=True)
        except aiohttp.ClientResponseError as e:
            error_message = str(e)

            expects_object_docs = (
                e.status == 422
                and "documents" in error_message
                and (
                    "model_attributes_type" in error_message
                    or "valid dictionary or object" in error_message
                )
            )

            if not expects_object_docs:
                logger.error(f"Rerank API error {e.status}: {error_message}")
                raise

            logger.warning(
                "Rerank API rejected string documents with 422; retrying with object documents format"
            )
            ...
            if request_format == "aliyun":
                fallback_input = dict(fallback_payload.get("input", {}))
                fallback_input["documents"] = object_docs
                fallback_payload["input"] = fallback_input
            else:
                fallback_payload["documents"] = object_docs

            return await _post_with_payload(fallback_payload)

    response_json = await _request_with_object_documents_fallback(payload, documents)
