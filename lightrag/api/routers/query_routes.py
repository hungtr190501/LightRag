...
def _extract_cited_reference_ids(content: str) -> list[str]:
    """Extract cited reference ids from model output in stable order.

    Supports multiple citation formats:
    - Inline: [1], [1,2], [1-3]
    - Parenthetical: (1), (1,2)
    - Bullet list: "- [1] ...", "* [2] ..."
    - Reference list style: "[1]: ..."
    """
    if not content:
        return []
    ...
    # Pattern 1: Bullet/numbered list style: "- [3] ..." or "* [2] ..."
    for match in re.finditer(r"^\s*[-*]\s*\[(\d+)\]", content, flags=re.MULTILINE):
        ref_id = match.group(1)
        if ref_id not in seen:
            seen.add(ref_id)
            ids.append(ref_id)

    # Pattern 2: Inline citations with various formats
    # Matches: [1], [1,2], [1-3], [1, 2, 3]
    for match in re.finditer(r"\[(\d+)(?:[-,]\s*\d+)*\]", content):
        # Extract all numbers from the bracket
    ...
    # Pattern 3: Parenthetical citations: (1), (1,2), (1-3)
    for match in re.finditer(r"\((\d+)(?:[-,]\s*\d+)*\)", content):
        nums = re.findall(r"\d+", match.group(0))
        for num in nums:
            if num not in seen:
    ...
    # Pattern 4: Reference list at end: "[1]: ..." or "[1] ..."
    if not ids:  # Only if no inline citations found
        for match in re.finditer(r"^\[(\d+)\]\s*[:\-]", content, flags=re.MULTILINE):
            ref_id = match.group(1)
            if ref_id not in seen:
                seen.add(ref_id)
                ids.append(ref_id)
...
def _filter_references_by_citations(
    response_content: str, references: list[dict]
) -> list[dict]:
    """Return references that are actually cited by the answer text."""
    if not references:
        return []

    cited_ids = _extract_cited_reference_ids(response_content)
    if not cited_ids:
        return []
...
def _compute_line_span(full_text: str, chunk_text: str) -> tuple[int | None, int | None]:
    """Compute 1-based line span of a chunk within a full document text."""
    if not full_text or not chunk_text:
        return None, None

    normalized_full = _normalize_newlines(full_text)
    normalized_chunk = _normalize_newlines(chunk_text).strip()
    ...
    if start_pos >= 0:
        ...
        start_line = normalized_full.count("\n", 0, start_pos) + 1
        end_line = normalized_full.count("\n", 0, max(end_pos - 1, start_pos)) + 1
        return start_line, end_line

    # Fallback: try matching by first non-empty line to get approximate start.
    first_line = ""
    ...
    if first_line:
        approx_start_pos = normalized_full.find(first_line)
        if approx_start_pos >= 0:
            start_line = normalized_full.count("\n", 0, approx_start_pos) + 1
            end_line = start_line + max(0, normalized_chunk.count("\n"))
            return start_line, end_line

    return None, None


async def _enrich_references_with_chunk_metadata(
    rag,
    data: dict[str, Any],
    include_chunk_content: bool = False,
) -> list[dict]:
    """Add chunk-level metadata (and optional content) to references.

    The enrichment adds chunk_id/chunk_order_index and best-effort line_start/line_end.
    """
    ...
    # Build reference_id -> chunk entries used by that reference.
    ref_to_chunks: dict[str, list[dict]] = {}
    for chunk in chunks:
        ref_id = str(chunk.get("reference_id", "") or "")
        if ref_id:
            ref_to_chunks.setdefault(ref_id, []).append(chunk)
    ...
    full_doc_by_id: dict[str, dict[str, Any]] = {}

    if unique_chunk_ids:
        try:
            chunk_meta_list = await rag.text_chunks.get_by_ids(unique_chunk_ids)
            for chunk_id, chunk_meta in zip(unique_chunk_ids, chunk_meta_list):
                if chunk_meta:
            ...
            for meta in chunk_meta_by_id.values():
                ...
                if isinstance(full_doc_id, str) and full_doc_id:
                    full_doc_ids.append(full_doc_id)

            unique_full_doc_ids = list(dict.fromkeys(full_doc_ids))
            if unique_full_doc_ids:
                full_doc_list = await rag.full_docs.get_by_ids(unique_full_doc_ids)
                for full_doc_id, full_doc_data in zip(unique_full_doc_ids, full_doc_list):
                    if full_doc_data:
                        full_doc_by_id[full_doc_id] = full_doc_data
        except Exception as exc:
            logger.warning(
                "Failed to enrich references with chunk/full-doc metadata: %s", exc
            )
    ...
    for ref in references:
        ...
        related_chunks = ref_to_chunks.get(ref_id, [])

        if include_chunk_content:
            content_list = [
                c.get("content", "")
                for c in related_chunks
                if isinstance(c.get("content"), str) and c.get("content")
            ]
            if content_list:
                ref_copy["content"] = content_list

        best_chunk = related_chunks[0] if related_chunks else None
        chunk_id = best_chunk.get("chunk_id") if best_chunk else None
        if isinstance(chunk_id, str) and chunk_id:
            ref_copy.setdefault("chunk_id", chunk_id)

            chunk_meta = chunk_meta_by_id.get(chunk_id, {})
            chunk_order_index = chunk_meta.get("chunk_order_index")
            if isinstance(chunk_order_index, int):
                ref_copy.setdefault("chunk_order_index", chunk_order_index)

            # Add semantic info if available
            sem_type = chunk_meta.get("semantic_type")
            sem_num = chunk_meta.get("semantic_number")
            if sem_type:
                ref_copy.setdefault("semantic_type", sem_type)
            if sem_num:
                ref_copy.setdefault("semantic_number", sem_num)
            ...
            full_doc_id = chunk_meta.get("full_doc_id")
            if isinstance(full_doc_id, str) and full_doc_id:
                full_doc_data = full_doc_by_id.get(full_doc_id, {})
                full_doc_content = full_doc_data.get("content", "")
                chunk_content = (
                    chunk_meta.get("content")
                    if isinstance(chunk_meta.get("content"), str)
                    else best_chunk.get("content", "") if best_chunk else ""
                )
                start_line, end_line = _compute_line_span(full_doc_content, chunk_content)
                if isinstance(start_line, int) and isinstance(end_line, int):
                    ref_copy["line_start"] = start_line
                    ref_copy["line_end"] = end_line
...
class ReferenceItem(BaseModel):
    ...
    chunk_order_index: Optional[int] = Field(
        default=None,
        description="Zero-based index of the cited chunk/segment in the source file",
    )
    chunk_id: Optional[str] = Field(
        default=None,
        description="Internal chunk identifier for the cited segment",
    )
    line_start: Optional[int] = Field(
        default=None,
        description="1-based start line of the cited segment in the source document",
    )
    line_end: Optional[int] = Field(
        default=None,
        description="1-based end line of the cited segment in the source document",
    )
    content: Optional[List[str]] = Field(
        default=None,
        description="List of chunk contents for this cited segment (only present when include_chunk_content=True)",
    )
        ...
        **Usage Examples:**
        ...
        try:
            ...
            if request.include_references:
                references = _filter_references_by_citations(response_content, references)
        ...
        try:
            ...
            async def stream_generator():
                ...
                if llm_response.get("is_streaming"):
                    # Streaming mode: emit chunks first, then emit grounded references.
                    # This avoids sending broad, ungrounded sources upfront.
                    if request.include_references:
                    ...
                    if request.include_references:
                        grounded_references = _filter_references_by_citations(
                            "".join(full_response_parts), references
                        )
                        yield f"{json.dumps({'references': grounded_references})}\n"
                else:
                    ...
                    if request.include_references:
                        references = _filter_references_by_citations(
        ...
        try:
            ...
            if isinstance(response, dict):
                if isinstance(response.get("data"), dict):
                    response["data"]["references"] = await _enrich_references_with_chunk_metadata(
                        rag,
                        response["data"],
