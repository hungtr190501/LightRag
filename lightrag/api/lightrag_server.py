...
def create_app(args):
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            # Initialize default workspace (also initializes its storages and migrates data)
            default_ws = args.workspace or "default"
            await workspace_manager.initialize_default(default_ws)

            # Update Ollama API to use the properly initialized default RAG instance
            default_rag_instance = await workspace_manager.get_rag(default_ws)
            ollama_api.rag = default_rag_instance

            # Initialize legal pipeline singletons (Qdrant + optional Neo4j)
            await _init_legal_singletons(default_rag_instance)

            # Initialize evaluation platform (SQLite + EvalManager)
            await _init_eval_singletons(default_rag_instance)
    ...
    # Build RAG factory (captures all LLM/embedding config; only workspace name varies)
    def rag_factory(workspace_name: str) -> LightRAG:
        try:
            return LightRAG(
            ...
            logger.error(f"Failed to initialize LightRAG for workspace '{workspace_name}': {e}")
            raise

    workspace_manager = WorkspaceManager(args.working_dir, rag_factory)

    default_workspace_name = args.workspace or "default"

    # FastAPI dependency: resolve RAG instance from LIGHTRAG-WORKSPACE header
    async def get_rag(request: Request) -> LightRAG:
        ws_name = request.headers.get("LIGHTRAG-WORKSPACE") or default_workspace_name
        return await workspace_manager.get_rag(ws_name)

    # FastAPI dependency: resolve DocumentManager from LIGHTRAG-WORKSPACE header
    async def get_doc_manager(request: Request) -> DocumentManager:
        ws_name = request.headers.get("LIGHTRAG-WORKSPACE") or default_workspace_name
        return workspace_manager.get_doc_manager(ws_name, args.input_dir)

    async def _init_legal_singletons(default_rag_instance):
        """Khởi tạo legal pipeline singletons: Qdrant + Neo4j (optional)."""
        ...
        if qdrant_host:
            try:
                ...
                # Dùng lại embedding_func từ LightRAG instance
                qdrant_storage = QdrantLegalVectorStorage.__new__(QdrantLegalVectorStorage)
                qdrant_storage.namespace = "legal_chunks"
                qdrant_storage.global_config = {
                    "working_dir": args.working_dir,
                ...
                qdrant_storage.embedding_func = default_rag_instance.embedding_func
                qdrant_storage.workspace = ""
                qdrant_storage.meta_fields = set()
                qdrant_storage.cosine_better_than_threshold = 0.2
                qdrant_storage.__post_init__()
                await qdrant_storage.initialize()
                logger.info("QdrantLegalVectorStorage initialized at %s", qdrant_host)
            except Exception as e:
                logger.warning("Qdrant init failed (legal search unavailable): %s", e)

        if neo4j_uri:
            try:
                from legal_rag.graph.builder import LegalGraphBuilder
                graph_builder = LegalGraphBuilder()
                await graph_builder.ensure_schema()
                logger.info("LegalGraphBuilder connected to Neo4j at %s", neo4j_uri)
            except Exception as e:
                logger.warning("Neo4j init failed (graph search unavailable): %s", e)

        set_legal_singletons(
            qdrant=qdrant_storage,
            rag=default_rag_instance,
            graph_builder=graph_builder,
        )

    async def _init_eval_singletons(default_rag_instance):
        """Initialize evaluation platform: SQLite database + EvalManager."""
        try:
            from legal_rag.eval.database import EvalDatabase
            from legal_rag.eval.eval_manager import EvalManager

            eval_db_path = os.path.join(args.working_dir, "eval", "eval.db")
            os.makedirs(os.path.dirname(eval_db_path), exist_ok=True)
            eval_db = EvalDatabase(eval_db_path)
            await eval_db.initialize()

            # Reuse legal singletons (Qdrant, Neo4j) if available
            qdrant_storage = None
            graph_builder = None
            try:
                from lightrag.api.routers.legal_routes import _qdrant_storage, _graph_builder
                qdrant_storage = _qdrant_storage
            ...
            set_eval_singletons(eval_db=eval_db, eval_manager=eval_manager)
            logger.info("EvalManager initialized with DB at %s", eval_db_path)
        except Exception as e:
            logger.warning("Eval system init failed (evaluation unavailable): %s", e)
    ...
    # The lifespan will replace ollama_api.rag with the properly initialized instance.
    _temp_rag = rag_factory(default_workspace_name)
    ollama_api = OllamaAPI(_temp_rag, top_k=args.top_k, api_key=api_key)
