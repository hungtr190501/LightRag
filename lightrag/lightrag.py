...
class LightRAG:
    ...
    The function should take the following parameters:
        ...
        - `semantic_chunking`: (optional) If True, split by legal semantic markers.
        - `use_langchain_semantic_chunker`: (optional) If True, use LangChain SemanticChunker.

    The function should return a list of dictionaries (or an awaitable that resolves to a list),
    where each dictionary represents a chunk with at least "tokens", "content", and "chunk_order_index" keys.
    If semantic_chunking is True, chunks may also have "semantic_type" and "semantic_number".
    """

    semantic_chunking: bool = field(
        default=get_env_value("SEMANTIC_CHUNKING", False, bool)
    )
    """Enable semantic chunking for legal text (Điều, Khoản, Điểm)."""

    use_langchain_semantic_chunker: bool = field(
        default=get_env_value("LANGCHAIN_SEMANTIC_CHUNKER", False, bool)
    )
    """Use LangChain SemanticChunker when semantic_chunking is enabled."""
                        ...
                        try:
                            ...
                            try:
                                chunking_result = self.chunking_func(
                                    ...
                                    use_langchain_semantic_chunker=self.use_langchain_semantic_chunker,
                                )
                            except TypeError as e:
                                if (
                                    'use_langchain_semantic_chunker' in str(e)
                                    or 'semantic_chunking' in str(e)
                                ):
                                    # Fallback to older custom chunking function signatures.
                                    try:
                                        chunking_result = self.chunking_func(
                                            self.tokenizer,
                                            content,
                                        ...
                                        if 'semantic_chunking' in str(inner_e):
                                            chunking_result = self.chunking_func(
                                                self.tokenizer,
                                                content,
                                                split_by_character,
