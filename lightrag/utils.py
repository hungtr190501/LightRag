...
def convert_to_user_format(
    ...
    for i, chunk in enumerate(chunks):
        chunk_data = {
            ...
            "semantic_number": chunk.get("semantic_number"),
        }
...
def generate_reference_list_from_chunks(
    ...
    """
    Generate reference list from chunks at chunk-level granularity.

    Each chunk gets its own reference_id so citations can point to a specific
    evidence segment instead of only a whole file.

    Reference ID is now stable: based on (file_path, chunk_id) or content hash.
    This ensures same chunk content always gets same reference_id across queries.
    """
    if not chunks:
        return [], []

    # Build deterministic, chunk-level references using stable keys
    # Priority: (file_path, chunk_id) -> (file_path, content_hash) -> (file_path, chunk_order_index) -> fallback index
    chunk_ref_to_id: dict[str, str] = {}
    reference_list: list[dict] = []

    def _build_stable_key(chunk: dict, index: int) -> str:
        """Build a stable key for a chunk based on available identifiers."""
        file_path = chunk.get("file_path", "") or "unknown_source"
        chunk_id = chunk.get("chunk_id", "")
        content = chunk.get("content", "")

        if chunk_id:
            return f"{file_path}::chunk_id::{chunk_id}"
        elif content:
            # Use content hash for stability
            content_hash = compute_mdhash_id(content)
            return f"{file_path}::content_hash::{content_hash}"
        else:
            # Fallback to chunk_order_index or index
            chunk_order_index = chunk.get("chunk_order_index")
            if chunk_order_index is not None:
                return f"{file_path}::order::{chunk_order_index}"
            else:
                return f"{file_path}::idx::{index}"

    for i, chunk in enumerate(chunks):
        chunk_key = _build_stable_key(chunk, i)
        if chunk_key in chunk_ref_to_id:
            # Same chunk content already seen, reuse reference_id
            continue

        ref_id = str(len(reference_list) + 1)
        chunk_ref_to_id[chunk_key] = ref_id
    ...
    for i, chunk in enumerate(chunks):
        ...
        chunk_key = _build_stable_key(chunk_copy, i)
        chunk_copy["reference_id"] = chunk_ref_to_id.get(chunk_key, "")
