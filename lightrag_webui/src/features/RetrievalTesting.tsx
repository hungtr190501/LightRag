...
import { EraserIcon, SendIcon, CopyIcon, ScaleIcon } from 'lucide-react'
...
const stripModelReferencesSection = (content: string): string => {
  return content
    .replace(/\n{0,2}###\s*(Tài liệu tham khảo|References)[\s\S]*$/im, '')
    .trim()
}
...
export default function RetrievalTesting() {
  ...
  const [selectedCitation, setSelectedCitation] = useState<{ ref: ReferenceItem; idx: string } | null>(null)
  const [lastQuery, setLastQuery] = useState('')

  const handleCitationClick = useCallback((ref: ReferenceItem, idx: string) => {
    setSelectedCitation({ ref, idx })
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        let capturedReferences: ReferenceItem[] = []
        ...
        if (state.querySettings.stream) {
          ...
          if (capturedReferences.length) {
            // Store references as structured data on the message (do NOT embed in markdown)
            const cleanedContent = stripModelReferencesSection(assistantMessage.content)
            assistantMessage.content = cleanedContent
            assistantMessage.references = capturedReferences

            const finalCotResult = parseCOTContent(cleanedContent)
            assistantMessage.thinkingContent = finalCotResult.thinkingContent
            assistantMessage.displayContent =
              finalCotResult.isThinking ? '' : (finalCotResult.displayContent || cleanedContent)
            assistantMessage.isThinking = finalCotResult.isThinking
          ...
          const cleanedContent = stripModelReferencesSection(response.response)
          if (response.references && response.references.length > 0) {
            assistantMessage.references = response.references
          }
          updateAssistantMessage(cleanedContent)
        }
      } catch (err) {
        // Handle error
        ...
        try {
          // Ensure references are persisted with the assistant message
          const finalAssistantMsg = { ...assistantMessage }
          useSettingsStore
            .getState()
            .setRetrievalHistory([...prevMessages, userMessage, finalAssistantMsg])
        } catch (error) {
  ...
  // Clear chat history when workspace changes (skip on initial mount)
  ...
  useEffect(() => {
    ...
    setLastQuery('')
    const name = currentWorkspace || 'default'
    toast.info(`Đã chuyển sang workspace "${name}" — lịch sử chat đã được xóa`)
  }, [currentWorkspace])
  ...
  return (
    <div className="flex size-full gap-2 px-2 pb-12 overflow-hidden">
      <div className="flex grow flex-col gap-3">
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center flex-1 gap-4 py-8 text-center select-none">
            <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center shadow-sm">
              <ScaleIcon className="w-7 h-7 text-primary" />
            </div>
            <div>
              <h2 className="text-lg font-semibold text-foreground">Trợ lý Tư vấn Pháp luật AI</h2>
              <p className="text-sm text-muted-foreground mt-1 max-w-sm">
                Hỏi về bất kỳ quy định pháp luật Việt Nam nào — nghị định, thông tư, luật, điều khoản cụ thể.
              </p>
              <p className="text-xs text-muted-foreground/60 mt-1">
                Workspace: <span className="font-medium text-muted-foreground">{currentWorkspace || 'default'}</span>
              </p>
            </div>
            <div className="flex flex-wrap gap-2 justify-center max-w-md">
              {[
                ...
                'Luật nào mới nhất về lao động nước ngoài?',
              ].map((suggestion) => (
                <button
                  key={suggestion}
                  onClick={() => setInputValue(suggestion)}
                  className="text-xs px-3 py-1.5 rounded-full border border-border/60 bg-muted/40 hover:bg-muted hover:border-primary/30 text-muted-foreground hover:text-foreground transition-all"
                >
                  {suggestion}
                </button>
              ))}
        ...
        {messages.length > 0 && (
          <div className="relative grow">
            <div
              ...
              className="absolute inset-0 flex flex-col overflow-auto rounded-xl border border-border/40 bg-muted/20 p-3 gap-1"
              onClick={() => {
                if (shouldFollowScrollRef.current) {
                  shouldFollowScrollRef.current = false;
                }
            ...
            >
              <div className="flex min-h-0 flex-1 flex-col gap-3">
                {messages.map((message) => (
                  <div
                    key={message.id}
                    className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'} items-end gap-1.5`}
                  >
                    {message.role === 'user' && (
                      <Button
                        onClick={() => handleCopyMessage(message)}
                        className="mb-1 size-6 rounded-md opacity-0 group-hover:opacity-60 hover:!opacity-100 transition-opacity shrink-0"
                    ...
                    <div className="group flex flex-col max-w-full">
                      <ChatMessage
                        message={message}
                    ...
                    {message.role === 'assistant' && (
                      <Button
                        onClick={() => handleCopyMessage(message)}
                        className="mb-1 size-6 rounded-md opacity-0 hover:opacity-100 transition-opacity shrink-0 self-end"
                        tooltip={t('retrievePanel.chatMessage.copyTooltip')}
                        variant="ghost"
                        size="icon"
                      >
        ...
        >
          ...
          <div className="flex-1 relative">
            ...
            {hasMultipleLines ? (
              <Textarea
                ...
                id="query-input"
                autoComplete="on"
                className="w-full min-h-[40px] max-h-[120px] overflow-y-auto rounded-xl"
                value={inputValue}
          ...
          <Button type="submit" variant="default" disabled={isLoading} size="sm" className="shrink-0 h-10 px-4">
      ...
      <CitationModal
        ...
        reference={selectedCitation?.ref ?? null}
        citationIndex={selectedCitation?.idx ?? ''}
        query={lastQuery}
      />
    </div>
  )
